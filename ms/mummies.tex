...
\jd{This section seems unfocused.} All of the results should be predictable based on the patterns above, once we understand how assumptions lead to estimates of R. The relationship between 1-individual results and 100-individual results is super-simple. Maybe we could present that once and try to explain instead of presenting over and over. For temperature, the key should be to figure out what factors drive R as a function of temperature -- that's sort of the point of the earlier part of the MS. We should figure out what the Numerical section is trying to learn or illustrate and rewrite it.}

We adopt the parameter descriptions presented in example 1, and simulate the model results using R studio (version 1.1.463) \cite{RStudioTeam2016}. We assume that key parameters are all temperature dependent and their relationship with temperature follows from \cite{Lord2018,PhelpsR.J.&Burrows,Hargrove2004a,Hargrove2019a}. \jd{It is not clear what you mean by “follows from”. Did you use parameters from all of those studies?}

\jd{Give your figures descriptive labels, or don't use labels at all.}
Figure \ref{fig:2} shows extinction probabilities for different counting points as a function of different levels of fixed temperatures, which the flies are experiencing. Extinction probabilities for the three counting points are plotted for the same range of temperature (15 \degree C - 33\degree C) and different initial population sizes. For temperatures within the survival limit for tsetse populations (16\degree C - 31\degree C) \cite{Are2019}, extinction probabilities are highest for populations of tsetse starting with a single larva. Extinction probabilities are least when we start with female adults in the larvipositing loop. Extinction probability is 1 for all counting points outside the range (16\degree C - 31\degree C)(Fig \ref{fig:2} A).  When the starting population size become larger, say 100, extinction probabilities approach 0 for temperatures in the range  16 - 31\degree C for all counting points (Fig \ref{fig:2}B).
